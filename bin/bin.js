...
var ASM_CONSTS = {
  119928: ($0) => { let idstr = UTF8ToString($0); let element = document.getElementById(idstr); return element.width; },  
 120030: ($0) => { let idstr = UTF8ToString($0); let element = document.getElementById(idstr); return element.height; },  
 120133: () => { return performance.now(); },  
 120163: ($0) => { let string = UTF8ToString($0); let overlay = document.getElementById("overlay"); overlay.textContent = string; },  
 120278: () => { let canvas = document.createElement("canvas"); canvas.id = "dummy-canvas"; canvas.style.display = "none"; document.body.appendChild(canvas); },  
 120423: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.RED; },  
 120577: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.R32F; },  
 120732: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.COMPRESSED_RGBA; },  
 120898: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.COMPRESSED_RED; }
};
...
function sound_play(_sound_id,loop) { const sound_id = UTF8ToString(_sound_id); var sound = document.getElementById(sound_id); if (!sound) return -1; var sound = document.getElementById(sound_id); sound.loop = !!loop; sound.play(); return 0; }
function sound_play2(_sound_id,volume,speed,loop,preserve_pitch) { const sound_id = UTF8ToString(_sound_id); var sound = document.getElementById(sound_id); if (!sound) return -1; sound.volume = volume; sound.playbackRate = speed; sound.preservesPitch = !!preserve_pitch; sound.loop = !!loop; sound.play(); return 0; }
function sound_stop(_sound_id) { const sound_id = UTF8ToString(_sound_id); var sound = document.getElementById(sound_id); if (!sound) return -1; sound.load(); return 0; }
function sound_pause(_sound_id) { const sound_id = UTF8ToString(_sound_id); var sound = document.getElementById(sound_id); if (!sound) return -1; sound.pause(); return 0; }
function sound_isfinished(_sound_id) { const sound_id = UTF8ToString(_sound_id); var sound = document.getElementById(sound_id); if (!sound) return false; return sound.ended; }
function sound_isplaying(_sound_id) { const sound_id = UTF8ToString(_sound_id); var sound = document.getElementById(sound_id); if (!sound) return false; return !sound.paused; }
function is_page_loaded() { return document.readyState === "complete"; }
function load_image_to_canvas(_canvas_id,_image_id) { const canvas_id = UTF8ToString(_canvas_id); const image_id = UTF8ToString(_image_id); const canvas = document.getElementById(canvas_id); if (!canvas) return -1; const context = canvas.getContext("2d"); if (!context) return -2; const image = document.getElementById(image_id); if (!image) return -3; canvas.width = image.width; canvas.height = image.height; context.drawImage(image, 0, 0); return 0; }
function load_canvas_to_buffer(buffer,_canvas_id,width,height) { const canvas_id = UTF8ToString(_canvas_id); const context = document.getElementById(canvas_id).getContext("2d"); if (!context) return -1; const image_data = context.getImageData(0, 0, width, height); HEAPU8.set(new Uint8Array(image_data.data.buffer), buffer); }
