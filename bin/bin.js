...
var ASM_CONSTS = {
  119896: ($0) => { let idstr = UTF8ToString($0); let element = document.getElementById(idstr); return element.width; },  
 119998: ($0) => { let idstr = UTF8ToString($0); let element = document.getElementById(idstr); return element.height; },  
 120101: () => { return performance.now(); },  
 120131: ($0) => { let string = UTF8ToString($0); let overlay = document.getElementById("overlay"); overlay.textContent = string; },  
 120246: () => { let canvas = document.createElement("canvas"); canvas.id = "dummy-canvas"; canvas.style.display = "none"; document.body.appendChild(canvas); },  
 120391: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.RED; },  
 120545: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.R32F; },  
 120700: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.COMPRESSED_RGBA; },  
 120866: () => { let canvas = document.getElementById("dummy-canvas"); let gl = canvas.getContext("webgl2"); if (!gl) gl = canvas.getContext("webgl1"); return gl.COMPRESSED_RED; }
};
